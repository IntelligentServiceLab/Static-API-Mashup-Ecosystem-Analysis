...
if __name__ == "__main__":
    ...
    coreList = []
    for i in range(27):
        k = nx.k_core(coperationNetwork,i)
        coreList.append(k)
    label = ['k-coerness', 'API number']
    text = []
    plt.figure(figsize=(20, 10))
    for j in range(27):
        text.append([j, len(nx.nodes(coreList[j]))])
    text = list(reversed(text))
    table = plt.table(cellText=text, colLabels=label, loc='center', cellLoc='center', rowLoc='center')
    plt.axis("off")
    plt.title("TABLE V. API NUMBER OF DIFFERENT K-CORES OF THE WEB API COLLABORATION NETWORK")
    plt.show()
